...
async function runSnapshotsForUser({ pool, userId, triggerType }) {
  ...
  const outputLanguage = settingsRows.length ? settingsRows[0].output_language : 'hinglish';
  ...
  for (const batch of batches) {
    ...
    const analysisResults = await analyzeMoments({
      moments: momentTexts,
      promptBody,
      outputLanguage
    });
    ...
    await withTransaction(pool, async (connection) => {
      ...
      if (findings.length > 0) {
        const values = findings.map((f) => {
          ...
          return [
            findingId, snapshotId, userId, f.type, f.topic, f.summary,
            f.recall_anchor || null, f.confidence_ai,
            JSON.stringify(f.evidence_moment_ids), 'unreviewed'
          ];
        });
